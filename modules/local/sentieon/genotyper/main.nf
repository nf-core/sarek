process SENTIEON_JOINT_GENOTYPING {
    ...
    // tuple val(meta), path("*.vcf.gz.tbi"), emit: vcf_gz_tbi // TO-DO: Apparently, not needed but should perhaps still be included in output?
    ...
    sentieon driver -r ${fasta}  --algo GVCFtyper ${gvcfs_str} ${prefix}.vcf.gz
