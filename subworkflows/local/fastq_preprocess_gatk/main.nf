include { BAM_APPLYBQSR                              } from '../../../subworkflows/local/bam_applybqsr'
include { BAM_APPLYBQSR_SPARK                        } from '../../../subworkflows/local/bam_applybqsr_spark'
include { BAM_BASERECALIBRATOR                       } from '../../../subworkflows/local/bam_baserecalibrator'
include { BAM_BASERECALIBRATOR_SPARK                 } from '../../../subworkflows/local/bam_baserecalibrator_spark'
include { BAM_CONVERT_SAMTOOLS as CONVERT_FASTQ_UMI  } from '../../../subworkflows/local/bam_convert_samtools'
include { BAM_MARKDUPLICATES                         } from '../../../subworkflows/local/bam_markduplicates'
include { BAM_MARKDUPLICATES_SPARK                   } from '../../../subworkflows/local/bam_markduplicates_spark'
include { BAM_MERGE_INDEX_SAMTOOLS                   } from '../../../subworkflows/local/bam_merge_index_samtools'
include { BAM_SENTIEON_DEDUP                         } from '../../../subworkflows/local/bam_sentieon_dedup'
include { CHANNEL_ALIGN_CREATE_CSV                   } from '../../../subworkflows/local/channel_align_create_csv'
include { CHANNEL_APPLYBQSR_CREATE_CSV               } from '../../../subworkflows/local/channel_applybqsr_create_csv'
include { CHANNEL_BASERECALIBRATOR_CREATE_CSV        } from '../../../subworkflows/local/channel_baserecalibrator_create_csv'
include { CHANNEL_MARKDUPLICATES_CREATE_CSV          } from '../../../subworkflows/local/channel_markduplicates_create_csv'
include { CRAM_QC_MOSDEPTH_SAMTOOLS as CRAM_QC_NO_MD } from '../../../subworkflows/local/cram_qc_mosdepth_samtools'
include { FASTP                                      } from '../../../modules/nf-core/fastp'
include { FASTQ_ALIGN                                } from '../../../subworkflows/local/fastq_align'
include { FASTQ_CREATE_UMI_CONSENSUS_FGBIO           } from '../../../subworkflows/local/fastq_create_umi_consensus_fgbio'
include { SAMTOOLS_CONVERT as BAM_TO_CRAM_MAPPING    } from '../../../modules/nf-core/samtools/convert'
...
workflow FASTQ_PREPROCESS_GATK {
    ...
    if (params.step == 'mapping') {
        ...
        if (params.save_mapped || ((params.skip_tools && params.skip_tools.split(',').contains('markduplicates')) && !(params.tools && params.tools.split(',').contains('sentieon_dedup')))) {
            ...
            if (params.save_output_as_bam) {
                ...
                CHANNEL_ALIGN_CREATE_CSV(BAM_TO_CRAM_MAPPING.out.cram.join(BAM_TO_CRAM_MAPPING.out.crai, failOnDuplicate: true, failOnMismatch: true), params.outdir, params.save_output_as_bam)
            }
    ...
    if (params.step in ['mapping', 'markduplicates']) {
        ...
        if (params.skip_tools && params.skip_tools.split(',').contains('markduplicates') && !(params.tools && params.tools.split(',').contains('sentieon_dedup'))) {
            if (params.step == 'mapping') {
        ...
        ch_md_cram_for_restart = Channel.empty()
            .mix(cram_markduplicates_no_spark, cram_markduplicates_spark, cram_sentieon_dedup)
    ...
    if (params.step in ['mapping', 'markduplicates', 'prepare_recalibration']) {
        ...
        if (params.step == 'prepare_recalibration') {
            ...
            ch_cram_for_bam_baserecalibrator = Channel.empty()
                .mix(ch_md_cram_for_restart, cram_skip_markduplicates)
                .map { meta, cram, crai -> [meta + [data_type: "cram"], cram, crai] }
        }
    ...
    if (params.step in ['mapping', 'markduplicates', 'prepare_recalibration', 'recalibrate']) {
        ...
        if (!(params.skip_tools && params.skip_tools.split(',').contains('baserecalibrator'))) {
            bam_variant_calling_no_spark = Channel.empty()
            bam_variant_calling_spark = Channel.empty()
            ...
            if (params.use_gatk_spark && params.use_gatk_spark.contains('baserecalibrator')) {
                ...
                bam_variant_calling_spark = BAM_APPLYBQSR_SPARK.out.bam
                ...
                bam_variant_calling_no_spark = BAM_APPLYBQSR.out.bam
            ...
            bam_variant_calling = Channel.empty()
                .mix(
                    bam_variant_calling_no_spark,
                    bam_variant_calling_spark,
                )

            // CSV should be written for the file actually out out, either CRAM or BAM
            csv_recalibration = Channel.empty()
            csv_recalibration = params.save_output_as_bam ? bam_variant_calling : cram_variant_calling
