...
workflow BAM_BASERECALIBRATOR {
    ...
    GATK4_BASERECALIBRATOR(
        ...
        fasta_fai,
        dict,
        known_sites.map{files -> [['id' : 'known_sites'], files]},
        known_sites_tbi.map{files -> [['id' : 'known_sites'], files]}
    )

    // Figuring out if there is one or more table(s) from the same sample
    table_to_merge = GATK4_BASERECALIBRATOR.out.table.map{ meta, table -> [ groupKey(meta, meta.num_intervals), table ] }.groupTuple().branch{ items ->
