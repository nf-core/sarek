...
workflow BAM_VARIANT_CALLING_TUMOR_ONLY_MUTECT2_MS {
    ...
    ch_contamination_out_seg = CALCULATECONTAMINATION_MS.out.segmentation.map{ meta, seg -> [[id: meta.patient, patient: meta.patient, sex: meta.sex, num_intervals: meta.num_intervals], seg]}.groupTuple()
    ch_contamination_out_cont = CALCULATECONTAMINATION_MS.out.contamination.map{ meta, cont -> [[id: meta.patient, patient: meta.patient, sex: meta.sex, num_intervals: meta.num_intervals], cont]}.groupTuple()
    ...
    mutect2_vcf_filtered = FILTERMUTECTCALLS.out.vcf.map{ meta, vcf -> [[patient:meta.patient, sex:meta.sex, num_intervals:meta.num_intervals, variantcaller:"mutect2_ms"], vcf]}
    mutect2_vcf_filtered_tbi = FILTERMUTECTCALLS.out.tbi.map{ meta, tbi -> [[patient:meta.patient, sex:meta.sex, num_intervals:meta.num_intervals, variantcaller:"mutect2_ms"], tbi]}
    mutect2_vcf_filtered_stats = FILTERMUTECTCALLS.out.stats.map{ meta, stats -> [[patient:meta.patient, sex:meta.sex, num_intervals:meta.num_intervals, variantcaller:"mutect2_ms"], stats]}
