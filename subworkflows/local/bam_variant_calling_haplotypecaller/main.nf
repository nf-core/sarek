...
include { GATK4_MERGEVCFS as MERGE_HAPLOTYPECALLER } from '../../../modules/nf-core/gatk4/mergevcfs'
...
workflow BAM_VARIANT_CALLING_HAPLOTYPECALLER {
    ...
    haplotypecaller_vcf = GATK4_HAPLOTYPECALLER.out.vcf
        .map { meta, vcf_ ->
            [meta - meta.subMap('interval_name'), vcf_]
        }
        .branch {
    ...
    haplotypecaller_tbi = GATK4_HAPLOTYPECALLER.out.tbi
        .map { meta, tbi ->
            [meta - meta.subMap('interval_name'), tbi]
        }
        .branch {
            intervals: it[0].num_intervals > 1
            no_intervals: it[0].num_intervals <= 1
    ...
    haplotypecaller_bam = GATK4_HAPLOTYPECALLER.out.bam
        .map { meta, bam ->
            [meta - meta.subMap('interval_name'), bam]
        }
        .branch {
            intervals: it[0].num_intervals > 1
            no_intervals: it[0].num_intervals <= 1
    ...
    haplotypecaller_tbi = Channel.empty()
        .mix(
    ...
    BAM_MERGE_INDEX_SAMTOOLS(
        haplotypecaller_bam.intervals.map { meta, bam -> [groupKey(meta, meta.num_intervals), bam] }.groupTuple().mix(haplotypecaller_bam.no_intervals)
    )
