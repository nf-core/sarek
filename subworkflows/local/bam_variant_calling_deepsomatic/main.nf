...
workflow BAM_VARIANT_CALLING_DEEPSOMATIC {
    take:
    cram_normal         // channel: [mandatory] [ meta, cram_normal, crai_normal ]
    cram_tumor          // channel: [mandatory] [ meta, cram, crai ]
    ...
    cram_normal_intervals = cram_normal.combine(intervals)
        // Move num_intervals to meta map
        .map{ meta, cram, crai, intervals, num_intervals -> [ meta + [ num_intervals:num_intervals ], cram, crai ]}
    ...
    DEEPSOMATIC(cram_normal_intervals, cram_tumor, intervals_only, fasta, fasta_fai, [ [ id:'null' ], [] ])
