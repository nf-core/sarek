...
include { SENTIEON_APPLYVARCAL as SENTIEON_APPLYVARCAL_INDEL } from '../../../modules/nf-core/sentieon/applyvarcal'
include { SENTIEON_APPLYVARCAL as SENTIEON_APPLYVARCAL_SNP   } from '../../../modules/nf-core/sentieon/applyvarcal'
include { SENTIEON_GVCFTYPER                                 } from '../../../modules/nf-core/sentieon/gvcftyper'
include { SENTIEON_VARCAL as SENTIEON_VARCAL_INDEL           } from '../../../modules/nf-core/sentieon/varcal'
include { SENTIEON_VARCAL as SENTIEON_VARCAL_SNP             } from '../../../modules/nf-core/sentieon/varcal'
...
workflow BAM_JOINT_CALLING_GERMLINE_SENTIEON {
    ...
    if (variant_caller == 'sentieon_dnascope') {
        ...
        genotype_vcf = merged_vcf.map { meta, vcf ->
            [meta + [patient: "all_samples", variantcaller: 'sentieon_dnascope'], vcf]
        }
        genotype_index = merged_tbi.map { meta, tbi ->
            [meta + [patient: "all_samples", variantcaller: 'sentieon_dnascope'], tbi]
        }
    }
    else {
        ...
        // Rework meta for variantscalled.csv and annotation tools
        vqsr_input_snp = vqsr_input
            .join(SENTIEON_VARCAL_SNP.out.recal, failOnDuplicate: true)
        ...
        // Rework meta for variantscalled.csv and annotation tools
        vqsr_input_indel = SENTIEON_APPLYVARCAL_SNP.out.vcf
            .join(SENTIEON_APPLYVARCAL_SNP.out.tbi)
            .map { meta, vcf, tbi -> [meta + [id: 'joint_variant_calling'], vcf, tbi] }
        ...
        genotype_vcf = merged_vcf
            .join(vqsr_vcf_for_join, remainder: true)
            .map { _meta, joint_vcf, recal_vcf ->
        ...
        genotype_index = merged_tbi
            .join(vqsr_tbi_for_join, remainder: true)
            .map { _meta, joint_tbi, recal_tbi ->
        ...
        versions = versions.mix(SENTIEON_APPLYVARCAL_SNP.out.versions)
