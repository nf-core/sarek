workflow SAMPLESHEET_TO_CHANNEL {
    ...
    references.map { meta, _fasta ->
        ...
        else if (meta.intervals_bed && !meta.intervals_bed.endsWith("bed") && !meta.intervals_bed.endsWith("list")) {
    ...
    references.map { meta, _fasta ->
        if (tools && tools.split(',').contains('ascat')) {
            ...
            if (!meta.ascat_loci_gc && !meta.ascat_loci_rt) {
