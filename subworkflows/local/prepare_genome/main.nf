...
workflow PREPARE_GENOME {
    ...
    if (tools && tools.split(',').contains('bbsplit')) {
        if (bbsplit_index) {
            ...
            if (bbsplit_index.endsWith('.tar.gz')) {
                ch_bbsplit_index = UNTAR_BBSPLIT_INDEX([[id: 'bbsplit_index'], file(bbsplit_index, checkIfExists: true)])
                    .untar
        ...
        } else if (bbsplit_fasta_list) {
            // Build it from scratch if we have FASTA
            Channel
                .from(file(bbsplit_fasta_list, checkIfExists: true))
                .splitCsv(header: false, sep: ',') // Read in 2 column csv file: short_name,path_to_fasta
            ...
            ch_bbsplit_index = BBMAP_BBSPLIT(
                [[id: "build_index"], []],           // Empty reads for index building
                [],                                  // Empty reads
                fasta.map { _meta, fasta_-> fasta_ }, // Primary reference - extract path only
                ch_bbsplit_fasta_list,               // Additional references
                true                                 // Build index flag
            ).index
    ...
    emit:
    bcftools_annotations_tbi = TABIX_BCFTOOLS_ANNOTATIONS.out.tbi.map { _meta, tbi -> [tbi] }.collect() // path: bcftools_annotations.vcf.gz.tbi
    ...
    dbsnp_tbi                = TABIX_DBSNP.out.tbi.map { _meta, tbi -> [tbi] }.collect()                // path: dbsnp.vcf.gz.tbi
    ...
    germline_resource_tbi    = TABIX_GERMLINE_RESOURCE.out.tbi.map { _meta, tbi -> [tbi] }.collect()    // path: germline_resource.vcf.gz.tbi
