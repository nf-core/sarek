...
class UTILS {
    ...
    public static def get_assertion = { Map args ->
        ...
        def vcf_files = getAllFilesFromDir(outdir, include: ['**/*.vcf{,.gz}'], ignore: ['**/test{N,T}.germline.vcf{,.gz}', '**/*.{freebayes,freebayes.filtered.bcftools_filtered.vcf.gz}.vcf{,.gz}',  '**/*.varlociraptor.{vcf}{,.gz}'])
