...
if (params.tools && checkTools(params.tools, 'ascat')) {
...
if (params.tools && checkTools(params.tools, 'mutect2')) {
    if (!params.pon) {
...
if (!params.dbsnp && !params.known_indels) {
    if (params.step in ['mapping', 'markduplicates', 'prepare_recalibration', 'recalibrate'] && (!params.skip_tools || (params.skip_tools && !checkTools(params.skip_tools, 'baserecalibrator')))) {
    ...
    if (params.tools && (checkTools(params.tools, 'haplotypecaller') || checkTools(params.tools, 'sentieon_haplotyper'))) {
...
if (params.joint_germline && (!params.tools || !(checkTools(params.tools, 'haplotypecaller') || checkTools(params.tools, 'sentieon_haplotyper')))) {
...
if (params.joint_mutect2 && (!params.tools || !checkTools(params.tools, 'mutect2'))) {
...
if (params.tools && (checkTools(params.tools, 'ascat') || checkTools(params.tools, 'controlfreec'))) {
    input_sample.map{
        if (it[0].sex == 'NA' ) {
...
workflow SAREK {
    ...
    if (params.tools && checkTools(params.tools, 'cnvkit')) {
    ...
    if (params.step == 'mapping') {
        ...
        // QC
        if (!(params.skip_tools && checkTools(params.skip_tools, 'fastqc'))) {
            FASTQC(input_fastq)
        ...
        if (
            params.save_mapped ||
            (
                (params.skip_tools && checkTools(params.skip_tools, 'markduplicates')) &&
                !(params.tools && checkTools(params.tools, 'sentieon_dedup'))
            )
        ) {
    ...
    if (params.step in ['mapping', 'markduplicates']) {
        ...
        if (
            params.skip_tools &&
            checkTools(params.skip_tools, 'markduplicates') &&
            !(params.tools && checkTools(params.tools, 'sentieon_dedup'))
        ) {
            if (params.step == 'mapping') {
        ...
        } else if (params.tools && checkTools(params.tools, 'sentieon_dedup')) {
        ...
        // Create CSV to restart from this step
        csv_subfolder = (params.tools && checkTools(params.tools, 'sentieon_dedup')) ? 'sentieon_dedup' : 'markduplicates'
    ...
    if (params.step in ['mapping', 'markduplicates', 'prepare_recalibration']) {
        ...
        if (!(params.skip_tools && checkTools(params.skip_tools, 'baserecalibrator'))) {
    ...
    if (params.tools) {
        ...
        BAM_VARIANT_CALLING_GERMLINE_ALL(
            ...
            params.skip_tools && checkTools(params.skip_tools, 'haplotypecaller_filter'), // true if filtering should be skipped
    ...
    if (!(params.skip_tools && checkTools(params.skip_tools, 'versions'))) {
    ...
    if (!(params.skip_tools && checkTools(params.skip_tools, 'multiqc'))) {
