...
- [#2235](https://github.com/nf-core/sarek/pull/2235) - **CNVKit output file names have changed.** They are now derived from the input alignment file name rather than the sample name — e.g. `<sample>.recal.cnr` instead of `<sample>.cnr` for samples recalibrated within the pipeline (or the provided CRAM's base name when starting from `--step variant_calling`). This follows from CNVKit consuming the shared `CRAM_TO_BAM` output. The output directories are unchanged (still `variant_calling/cnvkit/<sample>/` and `.../<tumorsample_vs_normalsample>/`).
