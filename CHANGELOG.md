...
- [#XXXX](https://github.com/nf-core/sarek/pull/XXXX) - Preserve `groupKey` size hint into `groupTuple` in the sentieon haplotyper and dnascope subworkflows so per-sample MERGE*SENTIEON*\*\_VCFS / GVCFS emits progressively as each sample's intervals finish instead of bursting at end-of-haplotyper. Also fixes the `interval_name` → `intervals_name` typo in the branch step (was a silent no-op).
