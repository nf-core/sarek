...
- [#2257](https://github.com/nf-core/sarek/pull/2257) - Fix two cases that completed successfully without producing the requested output: `--tools parabricks_haplotypecaller` with `--joint_germline` now fails early instead of finishing without any variants, as joint genotyping is not wired up for this caller yet; and `--vep_loftee` no longer silently omits `LoF` annotations under `-profile conda`, where the plugin path was hardcoded to `/opt/conda` instead of being resolved from `$CONDA_PREFIX` at runtime
