...
- [#2235](https://github.com/nf-core/sarek/pull/2235) - **CNVKit output file names when starting from `--step variant_calling`.** When a pre-aligned CRAM/BAM is supplied via `--step variant_calling`, CNVKit output files are now named after the input file's base name rather than the sample ID (a side effect of CNVKit consuming the shared `CRAM_TO_BAM` output). Runs starting from FASTQ are unaffected — outputs remain named by sample. Output directories are unchanged (`variant_calling/cnvkit/<sample>/` and `.../<tumorsample_vs_normalsample>/`).
