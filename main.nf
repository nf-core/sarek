...
def helpMessage() {
    ...
    Other options:
        ...
        --ascat_ploidy               Use this parameter to overwrite default behavior from ASCAT regarding ploidy.
        --ascat_purity               Use this parameter to overwrite default behavior from ASCAT regarding purity.
...
process Ascat {
    ...
    """
    else if (params.ascat_purity && !params.ascat_ploidy)
    """
        for f in *BAF *LogR; do sed 's/chr//g' \$f > tmpFile; mv tmpFile \$f;done
        Rscript ${workflow.projectDir}/bin/run_ascat.r --tumorbaf ${bafTumor} --tumorlogr ${logrTumor} --normalbaf ${bafNormal} --normallogr ${logrNormal} --tumorname ${idSampleTumor} --basedir ${baseDir} --gcfile ${acLociGC} --gender ${gender}  --purity ${ascat_purity}
    """
    else if (!params.ascat_purity && params.ascat_ploidy)
    """
        for f in *BAF *LogR; do sed 's/chr//g' \$f > tmpFile; mv tmpFile \$f;done
        Rscript ${workflow.projectDir}/bin/run_ascat.r --tumorbaf ${bafTumor} --tumorlogr ${logrTumor} --normalbaf ${bafNormal} --normallogr ${logrNormal} --tumorname ${idSampleTumor} --basedir ${baseDir} --gcfile ${acLociGC} --gender ${gender}  --ploidy ${ascat_ploidy}
    """
    else
    """
        for f in *BAF *LogR; do sed 's/chr//g' \$f > tmpFile; mv tmpFile \$f;done
